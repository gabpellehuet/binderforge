...
def main():
    ...
    task_name = "custom_task"                           # task label = project name; no Complexa config needed
...
# hotspot_residues is always referenced in binder_generate.yaml so must always be set
    if hotspot_res:
        # REMOVED double quotes: Hydra expects [A17,A11,A44] when bypassing the shell
        hs_str = "[" + ",".join(str(r) for r in hotspot_res) + "]"
    else:
        hs_str = "[]"
